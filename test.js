...
async function generateAIInsights() {
    ...
    area.innerHTML = '<div style="display:flex;align-items:center;gap:8px;color:var(--accent);"><div class="online-dot" style="animation: dotPulse 1.5s infinite;"></div><span style="font-weight:600;">Analysing your portfolio via Gemini...</span></div>';
    ...
    try {
        ...
        Provide a concise, professional, and insightful 2-to-3 sentence financial recommendation based on this data. Format the text nicely.`;
        ...
        if (data.candidates && data.candidates[0].content.parts[0].text) {
            const reply = data.candidates[0].content.parts[0].text.replace(/\*\*/g, ''); // Basic markdown cleanup
            area.innerHTML = `<span style="color:#fff; font-weight:500;">${reply}</span>`;
            triggerHaptic('success');
        } else {
            area.innerHTML = '<span style="color:var(--red);">AI analysis currently unavailable. Check quota.</span>';
            triggerHaptic('error');
        }
    } catch (e) {
...
async function handleAIScan(e, type) {
    ...
    notify('Scanning image with AI...', 'info');
    triggerHaptic('medium');
    const reader = new FileReader();
    reader.onload = async function(evt) {
        const base64Img = evt.target.result.split(',')[1];
        try {
            const GEMINI_API_KEY = "AIzaSyCU6KyYWjUg7Iikf3XdYteCiJnbJ_2ZZCQ";
            const url = `https://generativelanguage.googleapis.com/v1beta/models/gemini-1.5-flash:generateContent?key=${GEMINI_API_KEY}`;
            ...
            let prompt = "";
            if(type === 'expense') {
                prompt = "Extract details from this receipt/bill. Return a raw JSON object exactly like this: {\"vendor\": \"Vendor Name\", \"amount\": 1500, \"date\": \"YYYY-MM-DD\"}. If you cannot find a date, use today's date. Return ONLY valid JSON, without any markdown formatting like ```json.";
            }
            ...
            const response = await fetch(url, {
                ...
                body: JSON.stringify({
                    contents: [{
                        parts: [
                            { text: prompt },
                            { inline_data: { mime_type: file.type, data: base64Img } }
                        ]
                    }]
                })
            });
            ...
            if(type === 'expense') {
                if(result.vendor) $('e_desc').value = result.vendor;
                if(result.amount) $('e_amount').value = result.amount; // Allow raw numbers to be formatted correctly
                if(result.date) $('e_month').value = result.date.substr(0,7);
                triggerHaptic('success');
                notify('Receipt parsed successfully!', 'success');
                // Re-trigger formatting logic on input if needed
                formatMoneyInput.call($('e_amount'), {target: $('e_amount')});
            }
            
        } catch(err) {
            console.error("AI Scan Error:", err);
            notify('AI Scan failed to read image clearly.', 'error');
            triggerHaptic('error');
        }
...
async function registerBiometrics() {
    ...
    try {
        ...
        const publicKey = {
            challenge: challenge,
            rp: { name: "WealthFlow Elite", id: window.location.hostname || "localhost" },
            user: { id: userId, name: "owner@wealthflow.app", displayName: "WealthFlow Owner" },
        ...
        const auth = DB.getObj('auth');
        auth.biometricId = rawId;
        DB.set('auth', auth);
        
        notify('Biometrics registered successfully!', 'success');
        triggerHaptic('success');
    } catch (err) {
        console.error("Biometric Reg Error:", err);
        notify('Biometric registration failed or cancelled.', 'error');
    }
}

async function loginWithBiometrics() {
    const auth = DB.getObj('auth');
    if (!auth.biometricId || !window.PublicKeyCredential) return;
    
    try {
        const challenge = new Uint8Array(32); crypto.getRandomValues(challenge);
        const binaryString = atob(auth.biometricId);
        ...
        notify('Biometric verification failed. Please use PIN.', 'warn');
    }
}
...
window.onload = async function() {
    ...
    firebase.auth().onAuthStateChanged(async (user) => {
        if (user) {
            ...
            clearInterval(pInt);
            if(fillEl) fillEl.style.width = '100%';
            splashText.textContent = "Authentication Required";
            
            setTimeout(() => {
                $('splash').classList.add('hide');
                $('authScreen').classList.add('show');
                showAuthView('authGoogle'); // Show Google login view
            }, 600);
        }
    });
...
function triggerProactiveEngagement() {
    ...
    notify(`${greeting}, Gaurawa! Checking your daily schedule...`, 'info');
...
function triggerHaptic(type = 'light') {
    if (!DB.getObj('settings', {haptics: true}).haptics || !navigator.vibrate) return;
    try {
        switch (type) {
...
function renderSettings() {
    ...
    $('settingsContent').innerHTML = `
        ...
        <div class="settings-section" style="background: linear-gradient(145deg, var(--card), var(--bg2)); border: 1px solid var(--border2);">
            ...
            <div class="setting-row">
                <div class="setting-info"><div class="setting-label">Biometric Key (WebAuthn)</div><div class="setting-desc">Link FaceID / TouchID / Passkey to your account.</div></div>
                <button class="btn btn-secondary btn-sm" onclick="registerBiometrics()">Register Device</button>
            </div>
            <div class="setting-row">
...
function generateLuxuryPDFReport() {
    ...
    // Generate blob URL for safe sharing locally
    const pdfBlob = doc.output('blob');
    const pdfUrl = URL.createObjectURL(pdfBlob);
    
    // Create an anchor and click it to trigger local download
    const a = document.createElement('a');
    a.href = pdfUrl;
    ...
    // Upload to Firebase Storage to get a shareable URL
    notify('Generating secure shareable link...', 'info');
    const storageRef = firebase.storage().ref();
    const fileName = `statements/WealthFlow_Statement_${new Date().getTime()}.pdf`;
    const fileRef = storageRef.child(fileName);
    
    fileRef.put(pdfBlob).then(snapshot => {
        ...
        if (navigator.share) {
            navigator.share({
                title: 'WealthFlow Elite Statement',
                text: 'Here is your generated financial statement from WealthFlow.',
                url: downloadURL
            }).catch(e => console.log('Share failed:', e));
        }
    }).catch(err => {
        console.error("Storage upload error:", err);
        notify('Downloaded locally. Cloud link unavailable (Check connection or auth).', 'error');
    });
}
    ...
    try {
        ...
        window.frInterval = setInterval(() => {
            ...
            if (count <= 0) {
                ...
                window.location.reload(); // Hard refresh to clear memory maps
            }
        }, 1000);
